...
from preprocessing import categorize_street, encode_street_type, encode_traffic_signal, encode_crossing, encode_source
...
selected_columns = ['Severity', 'Traffic_Signal', 'Crossing', 'Street', 'Source', 'Distance(mi)']
...
us_accidents['Source_Encoded'] = us_accidents['Source'].apply(encode_source)
us_accidents.drop(columns=['Street', 'Street_Type', 'Traffic_Signal', 'Crossing', 'Source'], inplace=True)
...
X = us_accidents[['Traffic_Signal_Flag', 'Crossing_Flag', 'Highway_Flag', 
                  'Distance(mi)', 'Source_Encoded']].values
...
class AccidentSeverityModel(nn.Module):
    def __init__(self):
        ...
        # TODO: Create the first fully connected layer (input: 5 features, output: 128 neurons)
